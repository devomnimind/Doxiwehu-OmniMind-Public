...
def test_tools_framework():
    ...
    console.print(f"✓ [green]Audit chain:[/green] {'✅ Valid' if chain_valid else '❌ Invalid'}")
...
def test_individual_agents():
    ...
    try:
        ...
        result = code_agent.run_code_task("List files in current directory", max_iterations=2)
    ...
    try:
        ...
        success = result.get('completed', False)
        console.print(f"  {'✅' if success else '❌'} ArchitectAgent: completed={success}")
    ...
    try:
        ...
        if plan.get('subtasks'):
            ...
            for i, subtask in enumerate(plan['subtasks'], 1):
                console.print(f"  {i}. [{subtask['agent']}] {subtask['description'][:80]}")
...
def test_rlaif_feedback():
    ...
    try:
        ...
        success = framework.execute_tool('collect_feedback',
        ...
        stored = framework.execute_tool('episodic_memory',
                                       action='store',
                                       data={'test': 'phase_6_validation'})
...
def main():
    ...
    console.print("[bold magenta]║     🧠 OmniMind Phase 6 - Integration Test Suite        ║[/bold magenta]")
