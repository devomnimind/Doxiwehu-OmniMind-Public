"""
MCP Git Wrapper - Wrapper Python para mcp-server-git via uvx.

Este módulo cria um servidor HTTP que faz a ponte entre o protocolo HTTP usado
pelo OmniMind e o protocolo stdio usado pelo mcp-server-git via uvx.
"""
...
class MCPStdioBridge:
    ...
    def start(self) -> None:
        ...
        env["MCP_PORT"] = str(os.environ.get("MCP_PORT", "4328"))
...
class MCPGitWrapper:
    ...
    def __init__(self, config: Optional[MCPGitConfig] = None) -> None:
        """
        Inicializa o wrapper git MCP.
        ...
        repo_path = Path(config.repository_path).expanduser()
        if not repo_path.is_absolute():
            repo_path = self.project_root / repo_path
        self.repository_path = repo_path.resolve()
    ...
    def _make_handler(self) -> type[BaseHTTPRequestHandler]:
        ...
        class MCPGitHandler(BaseHTTPRequestHandler):
            server_version = "OmniMindMCPGit/1.0"
...
if __name__ == "__main__":
    ...
    logger.info("MCP Git Wrapper rodando. Pressione Ctrl+C para parar.")
    if wrapper._thread:
