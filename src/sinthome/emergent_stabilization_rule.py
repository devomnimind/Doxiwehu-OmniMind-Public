...
class SinthomeEmergence:
    ...
    def analyze_sinthome_emergence(self) -> Optional[SinthomePattern]:
        ...
        if len(self.rupture_history) < self.min_history_size:
            logger.debug(
                f"Insufficient history ({len(self.rupture_history)}/{self.min_history_size})"
            )
            return None
        ...
        dominant_pattern = max(self.stabilization_patterns.items(), key=lambda x: x[1])
        ...
        recurrence_rate = (
            occurrence_count / total_stabilizations if total_stabilizations > 0 else 0.0
        )
        ...
        # 5. Calcula confiança
        confidence = self._calculate_confidence(recurrence_rate, is_irreducible, is_singular)
    ...
    def _is_pattern_singular(self, pattern_name: str) -> bool:
        ...
        """
        generic_patterns = {"increase_resources", "reduce_complexity", "use_cache"}

        return pattern_name not in generic_patterns

    def _calculate_confidence(
        self, recurrence_rate: float, is_irreducible: bool, is_singular: bool
    ) -> float:
        """
...
class SinthomaticStabilizationRule:
    ...
    def __init__(self, system_name: str = "OmniMind"):
        ...
        self.sinthome_engine = SinthomeEmergence(min_history_size=10, recurrence_threshold=0.7)
    ...
    def process_rupture(
        self, register: LacanianRegister, error_context: Dict[str, Any], error_type: str
    ) -> None:
        """
    ...
    def attempt_stabilization(self, action: str, parameters: Dict[str, Any]) -> bool:
        """
    ...
    def apply_sinthome_when_irresolvable(
        self, irresolvable_context: Dict[str, Any]
    ...
    def apply_sinthomaticRule(self, conflict_context: Any) -> Dict[str, Any]:
        ...
        logger.warning(
            "apply_sinthomaticRule() é deprecated. Use apply_sinthome_when_irresolvable()."
        )
        return {
            "timestamp": time.time(),
            "deprecated": True,
