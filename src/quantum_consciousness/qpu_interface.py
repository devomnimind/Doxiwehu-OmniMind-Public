...
class SimulatorBackend(QPUBackend):
    ...
    def __init__(self, num_qubits: int = 10) -> None:
        ...
        if QISKIT_AVAILABLE:
            ...
            self.simulator = None
            logger.warning(
                "qiskit_aer_not_available",
                msg="Install with: pip install qiskit-aer"
            )
...
class IBMQBackend(QPUBackend):
    ...
    def execute(
        ...
        if not self.is_available():
            logger.warning(
                "ibmq_not_available_fallback_to_simulator"
            )
...
class QPUInterface:
    ...
    def __init__(
        ...
        logger.info(
            ...
            active=self.active_backend.get_info().name,
    ...
    def _select_backend(self) -> QPUBackend:
    ...
    def get_active_backend_info(self) -> BackendInfo:
    ...
    def switch_backend(self, backend_type: BackendType) -> bool:
        ...
        logger.info(
            "backend_switched",
            new_backend=self.active_backend.get_info().name
        )
        return True
