...
@pytest.fixture
def analyst(tmp_path) -> PsychoanalyticAnalyst:
...
"""
    ...
    with pytest.MonkeyPatch().context() as m:
        ...
        m.setattr("src.agents.react_agent.ReactAgent._run_supabase_memory_onboarding", MagicMock())
...
def test_analyst_initialization(analyst: PsychoanalyticAnalyst):
    """Test that the agent initializes correctly."""
    assert analyst.mode == "psychoanalyst"
    assert analyst.llm is not None


def test_analyze_session_freudian(analyst: PsychoanalyticAnalyst):
    """Test the session analysis with the Freudian framework."""
    session_notes = "O paciente relata um sonho recorrente com seu pai, onde ele se sente pequeno e impotente."
    
    mock_response = {
        "hypothesis": "A hipótese é que o sonho reflete um complexo de Édipo não resolvido, com o pai como figura de autoridade castradora.",
        "resistance": "A tendência do paciente em minimizar a importância do sonho pode ser uma forma de resistência.",
        "key_elements": "Sonho recorrente, figura paterna, sentimento de impotência.",
        "observations": "Explorar a relação do paciente com figuras de autoridade em sua vida desperta.",
    }
    ...
    analyst.llm.invoke.return_value = mock_llm_output
...
def test_generate_abnt_report(analyst: PsychoanalyticAnalyst):
    """Test the generation of a structured report."""
    analysis_data = {
        "framework_used": "Lacaniano",
        "hypothesis": "O discurso do paciente opera na cadeia de significantes, onde o sintoma é uma metáfora de uma verdade não dita.",
        "resistance": "O silêncio em certos tópicos funciona como uma barra entre o significante e o significado.",
...
def test_parse_analysis_handles_errors(analyst: PsychoanalyticAnalyst):
