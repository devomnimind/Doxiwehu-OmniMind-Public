...
class DummyMemory:
    ...
    def store_episode(self, task: str, action: str, result: str, reward: float, metadata: Dict[str, Any]) -> None:
...
class DummyHelper:
    ...
    def fetch_page(self, collection: str, node_fields: List[str], first: int, after: Optional[str] = None):
        self.calls += 1
        if not self.pages:
        ...
        return type(
            "DummyPage",
            (),
            {"nodes": page["nodes"], "has_next": page["has_next"], "cursor": page["cursor"]},
        )()


def test_supabase_onboarding_processes_nodes() -> None:
    config = SupabaseConfig(url="https://supabase.test", anon_key="anon", service_role_key="service")
    memory = DummyMemory()
    helper = DummyHelper(
        pages=[
            {
                "nodes": [
                    {"id": "1", "content": "first", "metadata": {"task": "seed", "action": "import", "reward": 0.3}},
                ],
                "has_next": True,
                "cursor": "a",
            },
...
def test_supabase_onboarding_handles_error() -> None:
    ...
    onboarding = SupabaseMemoryOnboarding(config=config, memory=memory, helper=ErrorHelper([]))
