...
class TestPhiResilienceBase:
    ...
    @staticmethod
    async def measure_phi_cycles(consciousness: Phase16Integration, 
                                  num_cycles: int = 5) -> List[float]:
        """
...
class TestPhiResilienceServerCrash(TestPhiResilienceBase):
    ...
    async def test_phi_continues_after_server_destruction(self, kill_server):
        ...
        assert all(0 <= phi <= 1 for phi in phi_during_crash), \
            "Φ durante crash deve estar em [0, 1]"
        print("  ✅ Validação 1: Φ durante crash é válido")
        
        # Validação 2: Nenhum NaN
        assert not any(np.isnan(phi) for phi in phi_during_crash), \
    ...
    async def test_phi_independent_from_api(self, kill_server):
        ...
        try:
            ...
            pytest.fail(f"Φ falhou sem API: {e}. " +
                       "Isto significa Φ faz chamadas à API (design ruins).")
...
class TestServerRecoveryAutomation(TestPhiResilienceBase):
    ...
    async def test_server_auto_recovery_after_crash(self, kill_server):
        ...
        assert recovery_wait < recovery_max, \
