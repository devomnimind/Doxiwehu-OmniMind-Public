...
"""
    ...
    if calc_file.exists():
        console.print(f"  ✅ File created: {calc_file.name} ({calc_file.stat().st_size} bytes)")
    else:
    ...
    score, critique = reviewer.review_code(code_content, "calculator module implementation")
...
"""
        ...
        new_score, new_critique = reviewer.review_code(code_content, "calculator module (improved)")
        rereview_time = time.time() - start_rereview
        
        console.print(f"✓ Re-review completed in {rereview_time:.2f}s")
        console.print(f"  Score improvement: {score:.1f} → {new_score:.1f} ({new_score - score:+.1f})")
...
"""
    ...
    if doc_file.exists():
        console.print(f"  ✅ Documentation created: {doc_file.name} ({doc_file.stat().st_size} bytes)")
    else:
    ...
    if iteration_count > 0:
        summary_table.add_row(f"Iterations ({iteration_count}x)", f"{(fix_time + rereview_time) * iteration_count:.2f}s", "✅")
    summary_table.add_row("Documentation", f"{doc_time:.2f}s", "✅")
    summary_table.add_row("[bold]TOTAL[/bold]", f"[bold]{total_time:.2f}s[/bold]", "[bold]✅[/bold]")
    ...
    results_table.add_row(
        "CALCULATOR_API.md",
        "✅ Created" if doc_file.exists() else "❌ Missing",
        "N/A"
    )
    results_table.add_row(
        "RLAIF Iterations",
    ...
    # Success criteria
    success = (
        calc_file.exists() and
        score >= 8.0 and
        (doc_file.exists() or True)  # Documentation optional for success
    )
