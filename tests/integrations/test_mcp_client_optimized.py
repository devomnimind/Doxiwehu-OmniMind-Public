...
class TestContextEntry:
    ...
    def test_context_entry_size_calculation(self) -> None:
        ...
        small_entry = ContextEntry(
            key="small", value="x", timestamp=time.time()
        )
        large_entry = ContextEntry(
            key="large", value="x" * 1000, timestamp=time.time()
        )
...
class TestEnhancedMCPClient:
    ...
    def test_write_file(
        ...
        mock_mcp_instance = Mock()
        mock_mcp_instance.write_file.return_value = {"size": 100, "path": "/test/file.txt"}
        mock_mcp.return_value = mock_mcp_instance
