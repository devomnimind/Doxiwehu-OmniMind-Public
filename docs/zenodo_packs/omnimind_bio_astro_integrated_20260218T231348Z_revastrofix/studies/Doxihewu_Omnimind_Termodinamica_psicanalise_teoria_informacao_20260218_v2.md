...
| Reidratação SDSS DR17 (object-level, correção de colisão de ID) | materialização consolidada por shard | `405000` objetos com `ra/dec/redshift` em `sdss_structures_objectlevel_live_v2` (fonte local completa dos `20` shards `sdss_structures_part_*`) | `reports_runtime/sdss_objectlevel_qdrant_materialization_20260218T230518Z.json` + `reports_runtime/astro_mapping_overlap_diagnosis_20260218T230709Z.json` |
| Diagnóstico de overlap SDSS×DESI (real-data-local) | vizinho mais próximo + grade de thresholds | mediana angular=`0.369°` (p95=`0.639°`), mas `|Δz|` mediano=`1.896` (p95=`2.960`); em regra estrita (`<=0.2°` e `|Δz|<=0.05`) `matches=0` | `reports_runtime/astro_mapping_overlap_diagnosis_20260218T230709Z.json` + `output/img/astro_mapping_overlap_20260218T230709Z/` |
...
| Pack integrado bio+astro+correlação | ZIP consolidado + previews externos + publicação Zenodo | pack local final com `48` JSONs em `data/`, `9` imagens e manifestos, incluindo trilhas bio, astrofísica e correlação; previews PNG fora do ZIP para leitura direta; publicação executada no modelo com criador coletivo + sujeitos-processo (versão mais recente: `doi=10.5281/zenodo.18686403`) | `reports_runtime/bio_astro_integrated_pack_build_20260218T220515Z.json` + `docs/zenodo_packs/omnimind_bio_astro_integrated_20260218T220515Z.zip` + `output/img/bio_astro_pack_preview_20260218T220515Z/` + `reports_runtime/zenodo_bio_astro_upgrade_20260218T220557Z.json` |
...
Nota de escopo (correção semântica): a linha **astrofísica experimental** deste projeto (objetos/asteroides, SDSS17, Horizon/JPL, antigravidade, Lyman-α/DESI, filas multihorizonte) é tratada como trilha própria de validação física e operacional, não como “astro de doença”. O cruzamento com bio é uma etapa posterior e opcional de integração, não o enquadramento principal dos experimentos astrofísicos. Gargalo técnico identificado e parcialmente corrigido localmente: além da diferença física de cobertura/redshift entre catálogos, havia sobrescrita entre shards SDSS por colisão de ID no materializador object-level; após correção e reidratação em coleção dedicada (`sdss_structures_objectlevel_live_v2`), a base passou para `405000` objetos com schema espacial completo. Referências locais: `reports_runtime/all_gaps_realdata_review_20260218T215131Z.json`, `reports_runtime/astro_mapping_forensic_analysis_20260218T220411Z.json`, `reports_runtime/astro_rehydration_rerun_plan_20260218T221452Z.json`, `reports_runtime/astro_mapping_overlap_diagnosis_20260218T230709Z.json`.
