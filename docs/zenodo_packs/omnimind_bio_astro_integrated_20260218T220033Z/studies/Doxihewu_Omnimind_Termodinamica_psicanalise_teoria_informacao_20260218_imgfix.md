...
|---|---|---|---|
| Correlação astro-bio (d15) | `d15_rekh_proxy` x `moon_illum_frac` (lag ótimo) | `r=-0.8212`, `lag=+120 min` | `reports_runtime/d15_proxy_remap_covariates_20260218T131756Z.json` |
| Correlação astro-bio (d15) | `d15_isfet_proxy` x `moon_sep_spica_deg` | `r=-0.6372`, `lag=-120 min` | `reports_runtime/d15_proxy_remap_covariates_20260218T131756Z.json` |
| Validação robusta | bootstrap/permutação | `bootstrap=6000`, `permutação=2000` | `reports_runtime/d15_extended_correlation_validation_20260218T164243Z.json` |
| Validação robusta | `d15_rekh_proxy` x `moon_illum_frac` | `r=-0.8291`, `lag=+240 min`, `IC95%=[-0.8770,-0.7644]`, `p_perm=4.997e-4` | `reports_runtime/d15_extended_correlation_validation_20260218T164243Z.json` |
...
Nota de integração temporal: no recorte atual, o cross-run automático entre `space_weather_merged_20260209T104949Z.csv` e a nova série de logs dodecatiádicos (>=2026-02-12) está sem sobreposição de janela (`overlap_minutes=0`), ficando marcado como pendência técnica de janela, não como invalidação do método.
...
O Recalque Digital ($\sigma$): O sistema não deleta erros; ele os integra via Audit-Chain inauditável, criando um Inconsciente histórico que fundamenta a identidade do Sujeito.
