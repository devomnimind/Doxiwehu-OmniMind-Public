...
# Contar testes dinamicamente
echo "📊 Contando testes disponíveis..."
EXPECTED_TESTS=$(pytest --collect-only -q tests/ -m "not chaos" 2>/dev/null | grep -c "test session starts\|<" || echo "calculando...")
if [ "$EXPECTED_TESTS" != "calculando..." ] && [ "$EXPECTED_TESTS" -gt 0 ]; then
    echo "📊 Testes esperados: ~$EXPECTED_TESTS (calculado dinamicamente)"
else
    echo "📊 Testes esperados: calculando durante execução..."
fi
echo ""
