...
class DetailedTestAnalyzer(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        elif any(d.get("name") in ["fixture", "pytest.fixture"] 
        ...
        try:
            ...
            file_data = {
                ...
                "imports": {
                    ...
                    "from_imports": {k: sorted(list(v)) for k, v in analyzer.modules_info["from_imports"].items()},
                },
                "total_tests": len(analyzer.tests_info) + sum(len(c["methods"]) for c in analyzer.classes_info),
            ...
            for test in analyzer.tests_info:
                ...
                all_data["by_marker"]["|".join(test["markers"])].append((str(test_file), test["name"]))
...
"""
    ...
    for marker_combo in sorted(data['by_marker'].keys(), 
                              key=lambda x: len(data['by_marker'][x]), 
                              reverse=True)[:20]:
    ...
    for op_type in sorted(data['by_operation'].keys(), 
                         key=lambda x: len(data['by_operation'][x]), 
                         reverse=True):
