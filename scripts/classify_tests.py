...
def main() -> None:
    ...
    print("=" * 100)
    if classifications:
        print(f"[MOCK]      (uses @patch)  : {mock_count:4d} ({100*mock_count//len(classifications):3d}%)")
        print(f"[SEMI-REAL] (real GPU/LLM) : {semi_count:4d} ({100*semi_count//len(classifications):3d}%)")
        print(f"[REAL]      (full system)  : {real_count:4d} ({100*real_count//len(classifications):3d}%)")
    print(f"{'=' * 100}")
