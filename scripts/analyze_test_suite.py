...
class TestAnalyzer(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        if node.name.startswith("test_"):
            ...
            if any(d.get("name") == "pytest.fixture" or d.get("name") == "fixture" 
        ...
        for decorator in node.decorator_list:
            ...
            if isinstance(decorator, ast.Name):
                ...
                if decorator.keywords:
                    dec_info["kwargs"] = {kw.arg: ast.unparse(kw.value) 
...
def categorize_tests(all_results: List[Dict]) -> Dict[str, Any]:
    ...
    for result in all_results:
        ...
        for test in result["tests"]:
            ...
            if any(d.get("name") == "mark" and "gpu" in str(d) 
            ...
            if any(d.get("name") == "mark" and "mock" in str(d) 
                   for d in test["decorators"]):
                categories["by_type"]["mock"].append(test_entry)
            
            if any(d.get("name") == "mark" and "real" in str(d) 
                   for d in test["decorators"]):
                categories["by_type"]["real"].append(test_entry)
            
            if any(d.get("name") == "mark" and "chaos" in str(d) 
                   for d in test["decorators"]):
...
""".format(
        ...
        avg_tests_per_file=categories["statistics"]["total_tests"] / max(1, categories["statistics"]["total_files"]),
    )
    ...
    for op_type, tests in sorted(categories["by_operationality"].items(), 
                                 key=lambda x: len(x[1]), reverse=True):
        if tests:
            percentage = (len(tests) / total * 100) if total > 0 else 0
            report += f"| {op_type.replace('_', ' ').title()} | {len(tests)} | {percentage:.1f}% |\n"
