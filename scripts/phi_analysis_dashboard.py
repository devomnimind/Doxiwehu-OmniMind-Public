...
def analyze_phi_metrics(file_path: str) -> None:
    ...
    print(f"Φ_média              : {format_float(stats.get('phi_mean', 0))} ± {format_float(stats.get('phi_std', 0))}")
    print(f"Φ_mínimo             : {format_float(stats.get('phi_min', 0))}")
    print(f"Φ_máximo             : {format_float(stats.get('phi_max', 0))}")
    print(f"Valores válidos [0,1]: {stats.get('bounded_count', 0)}/{stats.get('total_measurements', 0)}")
    ...
    if by_test:
        ...
        for test_name, test_stats in sorted_tests:
            ...
            print(f"  ({count:3} medições, range: [{format_float(min_val)}, {format_float(max_val)}])\n")
    ...
    if stats.get('phi_mean', 0) < 0.3:
        print(f"⚠️  Φ_média baixo ({format_float(stats.get('phi_mean', 0))}) - sistema pode estar desconsciente")
    elif stats.get('phi_mean', 0) > 0.8:
        print(f"✅ Φ_média alto ({format_float(stats.get('phi_mean', 0))}) - sistema bem consciente")
    else:
