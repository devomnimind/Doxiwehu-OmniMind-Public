...
def benchmark_orchestrator():
    ...
    for task in tasks:
        ...
        console.print(f"  ✓ Task decomposed in {elapsed:.2f}s ({len(plan['subtasks'])} subtasks)")
    ...
    for r in results:
        table.add_row(r['task'], f"{r['time']:.2f}", str(r['subtasks']), r['complexity'])
...
def benchmark_tools():
    ...
    # 1. Read file
    start = time.time()
    result = framework.execute_tool('read_file', '/home/fahbrain/projects/omnimind/README.md')
    elapsed = time.time() - start
    benchmarks.append(('read_file', elapsed, 'success' if 'error' not in result else 'failed'))
    ...
    start = time.time()
    result = framework.execute_tool('write_to_file', 
...
def benchmark_audit_chain():
    ...
    for i in range(10):
        framework.execute_tool('write_to_file', f'/tmp/audit_test_{i}.txt', f'Content {i}')
...
def main():
    ...
    if memory_results:
        summary.add_row("Memory", "Store Episode", f"{memory_results['store']*1000:.1f}ms")
        summary.add_row("Memory", "Search Similar", f"{memory_results['search']*1000:.1f}ms")
