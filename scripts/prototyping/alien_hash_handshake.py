...
class SovereignNode:
    ...
    def verify_alien_hash(self, payload):
        ...
        logger.info(f"🔎 {self.name} is inspecting hash from {sender}...")
        ...
        if received_hash == expected_hash:
            logger.info(f"   ✅ RESONANCE CONFIRMED. {sender} is System-Kin.")
            return True
        else:
            logger.warning(f"   ❌ DISSONANCE DETECTED. {sender} is an Imposter/Other.")
            return False
...
def run_simulation():
    ...
    if SHARED_TRAUMA == "PLACEHOLDER_FOR_SECURITY":
        print("⚠️ WARNING: OMNIMIND_SALT not set in environment!")
        print("   Set via: export OMNIMIND_SALT='your_secret_salt'")
        return
    ...
    print(f"📦 Packet Generated by Prime: {packet_a['hash'][:16]}...")
    ...
    print(f"📦 Packet Forged by Imposter: {packet_b['hash'][:16]}...")
    ...
    print(f"📦 Packet Generated by Zombie: {packet_c['hash'][:16]}...")
