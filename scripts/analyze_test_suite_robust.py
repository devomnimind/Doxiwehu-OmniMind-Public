...
class RobustTestAnalyzer:
    ...
    def _extract_fixtures(self):
        ...
        for match in re.finditer(pattern, self.content, re.MULTILINE):
            ...
            # Tenta extrair scope
            scope_match = re.search(r'@(?:pytest\.)?fixture\s*\(\s*scope\s*=\s*["\'](\w+)["\']', match.group(0))
    ...
    def _detect_operations(self):
        ...
        operations_patterns = {
            "http_requests": [r'requests\.(get|post|put|delete|patch)',
                             r'aiohttp\.ClientSession',
                             r'urllib\.request',
                             r'httpx\.'],
            "gpu_operations": [r'\.cuda', r'torch\.cuda', r'GPU', r'\.to\(.*?device',
                              r'cuda\.synchronize'],
            "file_io": [r'open\(', r'Path\(', r'\.read_text', r'\.write_text',
                       r'json\.load', r'yaml\.load'],
            "database": [r'\.query\(', r'\.execute\(', r'session\.',
                        r'ORM', r'Database\.'],
            "async_operations": [r'await ', r'asyncio\.',
                                r'aiofiles\.', r'concurrent\.futures'],
            "mocking": [r'@mock\.patch', r'@patch\(', r'MagicMock\(',
                       r'Mock\(', r'unittest\.mock'],
            "subprocess": [r'subprocess\.', r'Popen\('],
            "threading": [r'Thread\(', r'Lock\(', r'Event\(',
                         r'concurrent\.'],
            "external_services": [r'requests\.', r'boto3', r'azure\.',
                                 r'google\.'],
        }
...
def generate_report(data: Dict[str, Any]) -> str:
    ...
    report = f"""# 📚 COMPLETE & COMPREHENSIVE TEST SUITE ANALYSIS
...
"""
    ...
    total_ops = sum(data['by_operation'].values())
    for op_type in sorted(data['by_operation'].keys(), 
                         key=lambda x: data['by_operation'][x], 
                         reverse=True):
    ...
    if test_counts:
        report += f"### Test Count Distribution\n\n"
